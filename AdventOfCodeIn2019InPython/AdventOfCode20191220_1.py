...
class PortalLabyrinth:
    ...
    def _find_portals(self):
        ...
        for portal in grouped_portal_data:
            if portal == "AA":
                point = grouped_portal_data[portal][0]
                self._entry_points.append(point)
            elif portal == "ZZ":
                point = grouped_portal_data[portal][0]
                self._keys[point] = portal  # Actually the goal point. Allows to reuse the find all keys logic.
            else:
                side1 = grouped_portal_data[portal][0]
                side2 = grouped_portal_data[portal][1]
                self._portals[side1] = side2
                self._portals[side2] = side1

    def _portal_data(self, portal_point : Point) -> Optional[Tuple[str, Point]]:
        base_point = None
        ...
        return (portal_specifier, base_point)
    ...
    def _grouped_portal_data(self, portal_data : List[Tuple[str, Point]]) -> Dict[str, List[Point]]:
        grouped_data = {}
        for specifier, point in portal_data:
            grouped_data.setdefault(specifier, []).append(point)
        return grouped_data
    ...
    def has_portal(self, point : Point) -> bool:
        return point in self._portals

    def portal_destination(self, point : Point) -> Optional[Point]:
        return self._portals.get(point, None)
...
class PartialPath:
    def __init__(self, found_keys : Set[str], positions : Tuple[Point,Point,Point,Point], path_length : int):
        self.found_keys = found_keys
        self.positions = positions
        self.path_length = path_length

    def extended_path(self, new_key : str, index : int, position : Point, path_length_to_key : int) -> PartialPath:
...
class PortalMazeSolver:
    def shortest_all_key_path_length(self, maze : PortalLabyrinth) -> int:
    ...
    def find_shortest_path_to_reachable_keys(self, maze : PortalLabyrinth, partial_path : PartialPath, index : int) -> List[PartialPath]:
        ...
        while not outermost_points.empty() and len(seen_keys) < maze.number_of_keys():
            ...
            points_to_move_to = self._adjacent_points(current_position)
            if maze.has_portal(current_position):
                points_to_move_to.append(maze.portal_destination(current_position))
            for point in points_to_move_to:
                if not point in known_distances and maze.is_walkable(point):
                    if not maze.has_door(point) or maze.required_key(point) in collected_keys:
                        ...
                        if maze.has_key(point) and not maze.key(point) in collected_keys:
                            key = maze.key(point)
                            new_partial_path = partial_path.extended_path(key, index, point, known_distances[point])
...
def main():
    ...
    labyrinth = PortalLabyrinth(labyrinth_map)
