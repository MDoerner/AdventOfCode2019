...
class ArcadeGame:
    ...
    def auto_play(self):
        self.reset_game()
        self._paint()
        while not self._game_state.has_shut_down() and not self._is_won():
            next_move = self.next_auto_move()
            self._process_input([next_move])
            if self._is_in_safe_state():
                self._paint() 
        self._paint()

    def next_auto_move(self) -> JoystickPosition:
        (x_p, y_p) = self._display_state.paddle_position()
        (x_b, y_b) = self._display_state.ball_position()
        return JoystickPosition.Neutral if x_b == x_p else JoystickPosition.Left if x_b < x_p else JoystickPosition.Right

    def _is_in_safe_state(self) -> bool:
        (x,y) = self._display_state.paddle_position()
        return self._display_state.ball_position() == (x,y-1) \
            or self._display_state.ball_position() == (x-1,y-1) \
            or self._display_state.ball_position() == (x+1,y-1)

    def _is_won(self) -> bool:
        return self._display_state.number_of_items(DisplayItem.Block) == 0
