...
class MazeSolver:
    ...
    def find_shortest_path_to_reachable_keys(self, maze : Labyrinth, partial_path : PartialPath, index : int) -> List[PartialPath]:
...
def main():
    ...
    _adjust_map(labyrinth)
    #print(labyrinth.show())
    shortest_path_length = solver.shortest_all_key_path_length(labyrinth)
    print(shortest_path_length)


def _adjust_map(labyrinth : Labyrinth):
    for (x,y) in labyrinth.entry_points().copy():
        del labyrinth._corridors[(x,y)]
        labyrinth._entry_points.remove((x,y))
        for point in _adjacent_points((x,y)):
            if labyrinth.is_walkable(point):
                del labyrinth._corridors[point]
        labyrinth._corridors[(x+1,y+1)] = '@'
        labyrinth._corridors[(x-1,y+1)] = '@'
        labyrinth._corridors[(x+1,y-1)] = '@'
        labyrinth._corridors[(x-1,y-1)] = '@'
        labyrinth._entry_points.append((x+1,y+1))
        labyrinth._entry_points.append((x-1,y+1))
        labyrinth._entry_points.append((x+1,y-1))
        labyrinth._entry_points.append((x-1,y-1))


def _adjacent_points(point : Point) -> List[Point]:
    (x,y) = point
    return [(x-1,y), (x+1,y), (x,y-1), (x, y+1)]
